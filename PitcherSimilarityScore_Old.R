...
dist.vec <- mapply(FUN = pitcherSimilarity, p1 = rep(pnames, length(pnames)), p2 = rep(pnames, each = length(pnames)))

dist.mat <- matrix(dist.vec, nrow = length(pnames), ncol = length(pnames))
colnames(dist.mat) <- pnames
rownames(dist.mat) <- pnames

# Which pitchers are most similar?
which(dist.mat == min(dist.mat[dist.mat > 0]), arr.ind = T)

#plotSimilarity("Stephen Strasburg", "Matt Harvey")
plotSimilarity("Dylan Bundy", "Matt Garza")

# Hierarchical clustering
clusters <- hclust(as.dist(dist.mat), method = "complete")
plot(clusters)

plotSimilarity("James Shields", "Miguel Gonzalez")
...
clusters.df <- data.frame(name = pnames, cluster = class)
...
dist.mat.mds <- matrix(0, nrow = length(pnames), ncol = length(pnames))
...
compare.df <- data.frame(p1 = pnames, p2 = rep(pnames, each = length(pnames)),
